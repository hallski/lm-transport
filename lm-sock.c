...
_lm_sock_library_init (void)
{
    ...
    g_print ("Socket library initialising...\n");
    
#ifdef G_OS_WIN32
    g_print ("Checking for winsock 2.0 or above...\n");
...
_lm_sock_library_shutdown (void)
{
    ...
    g_print ("Socket library shutting down...\n");
...
void
_lm_sock_set_blocking (LmSocketHandle sock, 
                       gboolean block)
{
    int res;
    ...
    if (res != 0) {
        g_print ("Could not set connection to be %s\n",
                 block ? "blocking" : "non-blocking");
    }
}

void
_lm_sock_shutdown (LmSocketHandle sock)
{
    shutdown (sock, LM_SHUTDOWN);
}

void
_lm_sock_close (LmSocketHandle sock)
{
#ifndef G_OS_WIN32
    close (sock);
#else  /* G_OS_WIN32 */
...
void 
_lm_sock_get_error (LmSocketHandle   sock, 
                    void      *error, 
                    socklen_t *len)
{
...
gboolean
_lm_sock_set_keepalive (LmSocketHandle sock, int delay)
{
...
gchar *
_lm_sock_get_local_host (LmSocketHandle sock)
{
